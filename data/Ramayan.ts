...
export const getFeaturedVideos = (): Video[] => {
  return ramayanVideos.filter(v => v.featured === true);
};

export const getLatestVideos = (): Video[] => {
  return [...ramayanVideos]
...
export const getPopularVideos = (): Video[] => {
  return ramayanVideos.filter(v => v.rating >= 7.5).slice(0, 10);
};
...
export const getVideosByGenre = (genre: string): Video[] => {
  return ramayanVideos.filter(v => v.genre.includes(genre));
};
...
export const getRecommendedVideos = (currentVideoId: string): Video[] => {
  ...
  if (!current) return ramayanVideos.slice(0, 10);
...
export const getUpcomingVideos = (): Video[] => {
  return ramayanVideos.filter(v => v.year >= 2025).slice(0, 10);
};
